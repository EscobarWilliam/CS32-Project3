...
class Game
{
    public:
        ...
        //If the game is over (i.e., the move member function has been called and returned false), set over to true; otherwise, set over to false and do not change anything else.
    
        //If the game is over, set hasWinner to true if the game has a winner, or false if it resulted in a tie. If hasWinner is set to false, leave winner unchanged; otherwise, set it to the winning side.
        bool move(Side s);
    ...
    private:
    //every game has a board, two players, and keeps track of whose turn it is.
        Board m_board;
        Player* m_south;
        Player* m_north;
        Side m_turn; //south side player always moves first
